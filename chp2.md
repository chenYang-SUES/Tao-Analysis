...
**证明：** 见随想 3。<span style="float: right">□</span>

2.2.5 证明命题 2.2.14.（提示：定义 $Q(n)$ 是关于 $n$ 的一个如下性质：$P(m)$ 对任意满足 $m_0 \leqslant m \leqslant n$ 的 $m$ 均为真；注意，当 $n < m_0$时，$Q(n)$ 为真，因为此时 $m$ 的取值范围为空）。

**证明：** 令 $Q(n)$ 是关于 $n$ 的一个如下性质：$P(m)$ 对任意满足 $m_0 \leqslant m \leqslant n$ 的 $m$ 均为真。如此一来，要证明对任意满足 $m \geqslant m_0$ 的自然数 $m$，$P(m)$ 为真，我们只需要证明对任意自然数 $n$，$Q(n)$一定为真即可。对 $n$ 采用归纳法，首先考虑最基本的情况 $n=0$。根据自然数的三歧性，我们有如下三种情况：$m_0 < n$、$m_0 = n$ 和 $m_0 > n$。由于 $m_0 = m_0 + n$，根据自然数的序的定义可知$m_0 \geqslant n$。若 $m_0 = n$，由于序是反对称的，$m=m_0$。由于任意满足 $m \geqslant m_0$的自然数 $m$，均有如下内容成立：$P(m')$ 对任意满足 $m_0 \leqslant m' <m$ 的自然数均为真，那么 $P(m)$ 为真。因此 $P(m_0)$（即 $Q(n)$）是一个空虚的真命题，因为当 $m = m_0$ 时，前提中的 $m'$ 取值范围为空。若 $m_0 > n$，$Q(n)$ 也是一个空虚的真命题，因为此时 $m$ 的取值范围为空。因此 $n=0$ 时结论成立。现在归纳性地假设 $Q(n)$ 为真，那么我们必须证明 $Q(n++)$ 为真。我们能证明 $m \leqslant n$，当且仅当 $m < n++$：一方面$n++ = n + 1$，根据命题 2.2.12 可知$n < n++$，又因为 $m \leqslant \leq$，所以 $m < n++$；另一方面，当 $m < n++$ 时根据命题 2.2.12 可知 $m++ \leqslant n++$，即 $m+1 \leqslant n+1$，再根据命题 2.2.12 可知 $m \leqslant n$。所以“$P(m)$ 对任意满足 $m_0 \leqslant m < n++$ 均为真” 是 $Q(n)$ 的等价命题，也为真。根据命题 2.2.14 的假设内容可知，$P(n++)$ 也为真，所以 $P(m)$ 对任意满足 $m_0 \leqslant m \leqslant n++$ 的 $m$ 均为真，即 $Q(n++)$，至此归纳过程就结束了。<span style="float: right">□</span>

2.2.6 令 $n$ 表示一个自然数，令 $P(m)$ 是关于自然数的一个性质并满足：只要 $P(m++)$ 为真，$P(m)$ 就为真。假设 $P(n)$ 也为真，证明：$P(m)$ 对任意满足 $m \leqslant n$的自然数 $m$ 均为真；这被称为**逆向归纳法原理**。（提示：对变量 $n$ 使用归纳法。）

**证明：** 令 $Q(n)$ 表示命题“若 $P(n)$ 为真，则 $P(m)$ 对任意满足 $m \leqslant n$ 的自然数 $m$ 均为真”。对 $n$ 采用归纳法。首先考虑最基本的情况，$n=0$。根据自然数的三歧性，存在三种可能的情况：$m < n$，$m > n$，$m=n$。前两种情况下 $m$ 的取值范围为空，因此 $P(m)$ 是空虚的真命题，第三种情况下 $P(n)$ 与 $P(m)$ 一样均为真，于是 $n=0$ 时结论成立。现在假设性地归纳 $Q(n)$ 为真，那么我么必须证明“若 $P(n++)$ 为真，则 $P(m)$ 对任意满足 $m \leqslant n++$ 的自然数均为真”。由题设可知，$P(n++)$ 为真可以推出 $P(n)$ 为真，进而可知 $Q(n++)$ 为真。至此整个归纳过程就结束了。<span style="float: right">□</span>


## 2.3 乘法

在上一节中，我们已经证明了所知道的所有关于加法和序的基本事实。为了节省篇幅以及避免赘述那些显然的事情，接下来我们将允许使用所熟悉的关于加法和序的代数法则，而不加进一步说明。于是我们可以写出 $a+b+c=c+b+a$ 这样的内容，不需要进一步解释说明。现在我们引入乘法。正如加法是重复的增量运算一样，乘法是重复的加法运算。

> **定义 2.3.1**（自然数的乘法）令 $m$ 表示任意一个自然数。我们定义 $0\times m := 0$ 来表示把 $0$ 乘到 $m$ 上。现在归纳性地假设已经定义了如何把 $n$ 乘到 $m$ 上，那么我们可以通过定义 $(n++) \times m:= (n \times m) + m$ 把 $n++$ 乘到 $m$ 上。

因此 $0 \times m =0$，$1 \times m = 0 + m$，$2 \times m = 0+m+m$，等等。利用归纳法容易证明任意两个自然数的乘积任然是自然数。

> **引理 2.3.2**（乘法是可交换的）令 $n$ 和 $m$ 表示任意连个自然数，那么有 $n \times m = m \times n$ 成立。

**证明：** 参见习题 2.3.1。<span style="float: right">□</span>

我们将把 $n \times m$ 简写为 $nm$，而且按照惯例，乘法运算优先于加法运算。于是，譬如 $ab + c$ 就意味着 $(a \times b) + c$，而不是 $a \times (b+c)$。[我们也将利用其他算术运算优先级的符号惯例（在它们被定义之后）来避免总是使用圆括号。]

> **引理 2.3.3** （正自然数没有零因子）设 $n$、$m$为自然数。那么 $n \times m=0$，当且仅当 $n$ 和 $m$ 中至少有一个为零。特别地，如果 $n$ 和 $m$ 均为正的，那么 $nm$ 也是正。

**证明：** 参见习题 2.3.2。<span style="float: right">□</span>

> **命题 2.3.4**（分配律）对任意自然数$a$、$b$、$c$，$a(b+c)=ab+ac$ 和 $(b+c)a = ba + ca$ 均成立。

**证明：** 由于乘法是可交换的，因此只需证明第一个等式 $a(b+c) = ab + ac$ 成立即可。固定 $a$ 和 $b$，并对 $c$ 进行归纳。我们来证明最基本的 $c=0$ 时的结论，也就是证明 $a(b+0)=ab+a0$。等式左端为 $ab$，而右端 $ab + 0=ab$，因此我们完成了对 $c=0$ 时结论的证明。现在我们归纳性地假设 $a(b+c)=ab + ac$成立，接下来要证明的是 $a(b + (c++))=ab + a(c++)$。等式左端为 $a((b+c)++)=a(b+c)+a$，右端为 $ab + ac + a=a(b+c)+a$，那么到这里归纳过程就结束了。<span style="float: right">□</span>

> **命题 2.3.5**（乘法是可结合的）对任意自然数 $a$、$b$、$c$，$(a \times b)\times c =a\times (b\times c)$ 均成立。

**证明：** 参见习题 2.3.3。

> **命题 2.3.6**（乘法保持序不变）如果 $a$、$b$ 是满足 $a < b$的自然数，并且 $c$ 是正的，那么 $ac < bc$。

**证明：** 因为 $a < b$，所以存在某个正自然数 $d$ 使得 $b = a+ d$。等式两端同时乘以 $c$，然后利用分配律可知，$bc=ac+dc$。由于 $d$ 是正的且 $c$ 也是正的。所以 $dc$ 是正的。于是得到 $ac < bc$。<span style="float: right">□</span>

> **推论 2.3.7**（消去律）设 $a、b、c$ 是自然数，满足 $ac=bc$ 且 $c$ 不为零，那么 $a=b$。

**注 2.3.8** 正如命题 2.2.6 提到“虚拟减法”一样，该推论给出了“虚拟除法”。其中“虚拟减法”使我们最终定义了真正的减法，而且在以后定义真正的除法时，我们也将会用到“虚拟除法”。

**证明：** 利用序的三歧性（命题 2.2.13），我们有如下三种情况：$a < b$、$a=b$ 和 $a > b$。首先假设 $a < b$，那么根据命题 2.3.6 可得，$ac < bc$，这与已知条件矛盾。当 $a > b$ 时能够推出类似的矛盾，于是唯一可能的情况是 $a=b$，结论得正。<span style="float: right">□</span>

利用这些命题很容易推导出大家熟知的所有包含加法和乘法在内的代数法则，例子参见习题 2.3.4。

既然我们有了熟悉的加法和乘法运算，增长这个相对更原始的概念将会逐渐被淘汰，并且从现在开始我们将会很少再见到这个概念。由于 $n++=n+1$，所以在任何情况下总是用加法运算来描述增量运算。

> **命题 2.3.9**（欧几里得算法）设 $n$ 是一个自然数，$q$ 表示一个正自然数，那么存在自然数 $m$ 和 $r$ 使得 $0 \leqslant r < q$ 并且 $n =mq+r$。

**注 2.3.10** 也就是说，我们可以用一个正自然数 $q$ 去除一个自然数 $n$，从而得到商 $m$（另一个自然数）和余数 $r$（比 $q$ 小）。该算法标志着**数论**的开始。数论是一门优美且重要的课程，但是它超出了本书的范围。

**证明：** 参见习题 2.3.5。

正如我们可以用增量运算递归地定义加法运算以及用加法运算递归地定义乘法运算一样，我们可以用乘法运算递归地定义**指数运算**。

> **定义 2.3.11**（自然数的指数运算）设 $m$ 是一个自然数。我们定义 $m^0:=1$ 来表示把 $m$ 升到 $0$ 次幂；特别地，定义 $0^0:=1$。现在递归地假设对于某个自然数 $n$，$m^n$ 已经被定义了，那么我们定义 $m ^{n++}:=m^n \times m$。

例 2.3.12 于是譬如 $x^1 = x^0 \times x = 1 \times x = x$，$x^2 = x^1 \times x = x \times x$，$x^3 = x^2 \times x= x \times x \times x$，以此类推。根据归纳法可知，这种递归地定义把所有自然数 $n$ 所对应的 $x^n$ 都定义了。

这里我们不对指数运算的理论进行深入研究，而要等到定义了整数和有理数的概念之后再说，参见命题 4.3.10。

## 习题

2.3.1 证明引理 2.3.2。（提示：修改引理 2.2.2、引理 2.2.3 以及命题 2.2.4的证明。）

**证明：** 令 $P$表示命题“对任意的自然数 $n$，$n \times 0 = 0$”，$Q$ 表示命题“对任意的自然数 $n$ 和 $m$，有 $m \times (n++) = m \times n + m$”。首先证明 $P$。对 $n$ 使用归纳法。当 $n=0$ 时，根据定义 2.3.1 可知 $0 \times 0 =0$。现在假设性地归纳 $n \times 0 = 0$，根据定义 2.3.1 可知 $(n++) \times 0 = n \times + 0 = 0 + 0 = 0$。至此整个归纳过程就结束了。其次证明 $Q$，保持 $n$ 不变，对 $m$ 采用归纳法，当 $m=0$ 时，我们必须证明 $0 \times (n++) = n \times 0 + m$。根据乘法的定义可知，$0 \times (n++) =0$；根据命题 $P$可知，$n \times 0 + m = 0+0=0$，所以要证明的等式的两端均等于 $0$，进而等式两端相等。现在归纳性地假设 $m \times (n++)= m \times n + m$，那么我们必须证明 $(m++) \times (n++) = (m++) \times n + (m++)$。根据定义 2.3.1 可知 $(m++) \times (n++) = m \times (n++) + (n++) = m \times n + m + (n++) = m \times n + n + (m++) = (m++) \times n + (m++)$。至此整个归纳过程就结束了。<span style="float: right">□</span>

2.3.2 证明引理 2.3.3。（提示：首先证明第二个命题。）

**证明：** 首先证明“如果 $n$ 和 $m$ 均为正的，那么 $nm$ 也是正的”。对 $n$ 采用归纳法，当 $n=0$ 时，前提为假，因此该命题是空虚的真命题。现在归纳性地假设 $nm$ 是正的，我们需要证明 $(n++)m$ 也是正的。根据乘法的定义 $(n++)m=(n++) \times m = nm + m$。根据命题 2.2.8 可知，$nm + m$ 是正的，即 $(n++)m$ 为正的。至此整个归纳过程就结束了。再证明“$n \times m$，当且仅当 $n$ 和 $m$ 中至少有一个为零”。先证明必要性，当 $n=0$ 时，根据定义 2.3.1 可知，$nm =0$；而当 $m=0$时，根据引理 2.3.2 和定义 2.3.1 可知，$nm=mn=0$。因此若 $n$ 和 $m$ 中至少有一个为零，则 $n \times m=0$；再证明充分性，假设当 $n \times m =0$ 时，$n$ 和 $m$ 均不为零，根据定义 2.2.7 可知 $n$ 和 $m$ 均为正的，再根据上一个证明可知，$nm$ 也是正的，这与 $nm=0$ 矛盾。因此 $n$ 和 $m$ 中至少有一个为零。<span style="float: right">□</span>

2.3.3 证明命题 2.3.5。（提示：修改命题 2.2.5 的证明并利用分配律。）

**证明：** 保持保持 $a$ 和 $b$ 固定，并对 $c$ 采用归纳法。当 $c=0$ 时，我们需要证明 $(a \times b) \times 1 = a \times (b \times 1)$。根据引理 2.3.2 和 定义 2.3.1 可知，$(ab) \times 1 = 1 \times (ab) = (0++) \times (ab) = 0 \times (ab) + ab = ab$，$a \times (b \times 1)= a \times (1 \times b) = a \times ((0++) \times b) = a \times (0 \times b +b) = ab$，因此我们证明了等式左端等于等式右端。现在归纳性地假设 $(a \times b) \times c = a \times (b \times c)$，我们需要证明 $(a \times b) \times (c++) = a \times (b \times (c++))$。根据引理 2.3.2、定义 2.3.1 和 命题 2.3.4，$(a \times b) \times (c++) = (c++) \times (a \times b) = c \times (a \times b) + (a \times b) = (a \times b) \times c + (a \times b) = a \times (b \times c) + a\times b = a \times (b\times c + b) = a \times (b \times (c + 1)) = a \times (b \times (c++))$。 至此整个归纳过程就结束了。<span style="float: right">□</span>

2.3.4 证明等式 $(a+b)^2 = a^2 + 2ab + b^2$ 对任意自然数 $a$ 和 $b$ 均成立。

**证明：** 保持 $b$ 不变，对 $a$ 采用归纳法。当 $a=0$ 时，我们必须证明 $(0 + b)^2 = 0^2 + 2 \times 0 \times b + b^2$。$(0 + b)^2 = b^2$，根据乘法的定义，$0^2 + 2 \times 0 \times b + b^2=b^2$，所以当 $a=0$ 时结论成立。现在归纳性地假设 $(a + b)^2 = a^2 + 2ab + b^2$，我们必须证明 $((a++) + b) ^2 = (a++)^2 + 2(a++)b + b^2$。根据定义 2.3.1 和命题 2.3.4 可知，$((a++)+b)^2=((a+b)++)^2=(a+b)((a+b)+1)+((a+b)+1)=(a+b)^2+(a+b)+(a+b)+1=(a+b)^2+2(a+b)+1$ 以及 $(a++)^2 + 2(a++)b + b^2 = a(a++)+(a++)+2(ab+b)+b^2=a^2+a+a+1+2ab+2b+b^2=(a+b)^2+2(a+b)+1$。所以，要证明的等式两端均等于 $(a+b)^2+2(a+b)+1$，进而等式两端相等。至此整个归纳过程就结束。

2.3.5 证明命题 2.3.9。（提示：固定 $q$ 并对 $n$ 进行归纳）

**证明：** 令 $Q(n)$ 表示命题“对于自然数 $n$，存在自然数 $m$ 和 $r$ 使得 $0 \leqslant r < q$ 并且 $n = mq+r$”。保持 $q$ 不变并对 $n$ 采用归纳法。当 $n=0$ 时，令 $m:=0$ 以及 $r:=0$。根据定义 2.2.7 可知 $q \neq 0$，由于$q = r + q$ 及 $r = r + r = 0$，根据定义 2.2.11可知，$0 \leqslant r < q$，根据乘法的定义，$n = mq + r = 0$，于是我们证明了$Q(0)$为真。现在归纳性地假设，$Q(n)$ 为真，我们必须证明 $Q(n++)$ 为真。根据命题 2.2.13，$r++ < q$、$r++=q$、$r++>q$，三种表述中恰有一种表示为真。又因为 $r < q$，根据根据命题 2.2.12 可知，$r++ \leqslant q$。当 $r++ < q$时，令$m_1:=m$ 及 $r_1 = r++$，由于 $r++ = r + 1$，所以 $r \leqslant r++$，由于序是可传递的（命题 2.2.12）, 因此 $0 \leqslant r++ <q$，而且 $n++ = n+1 = m_1q+r+1 = m_1q+r_1$。当 $r++ = q$ 时，令 $m_1:= m++$，$r_1:=0$，显然 $0 \leqslant r_1 < q$，根据乘法的分配律和交换律可知，$n++ = mq + r + 1 = mq + q = (m++)q = m_1q+r_1$。因此，我们证明了 $Q(n++)$ 为真，至此整个归纳过程就结束了。
